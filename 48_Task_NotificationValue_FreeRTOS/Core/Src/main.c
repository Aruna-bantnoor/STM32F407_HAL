...
int main(void)
{
  ...
  sprintf(user_msg,"Task Notification demo\n\r");
  printmsg(user_msg);
...
void vtask1_handler(void *params)
{
	...
	while(1)
	{
		// Lets wait until we receive notification event from button task
		if(xTaskNotifyWait(0, 0, &current_notification_value, portMAX_DELAY ) == pdTRUE)
		{
			// We received the notification , so toggle LED
			HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);

			sprintf(user_msg,"Notification is received: Button Press count: %ld \n\r", current_notification_value);
