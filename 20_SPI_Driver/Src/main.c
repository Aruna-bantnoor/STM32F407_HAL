...
int main()
{
    ...
    SPI2_Init(); //Configure SPI settings
    ...
    // enable SPI
    SPI_PeripheralControl(SPI2, ENABLE);// This sets SPE = 1

    delay();


    GPIO_WriteToOutputPin(GPIOB, GPIO_PIN_NO_12, RESET); // select slave, makes PB12 low
    SPI_TransmitReceive(SPI2, (uint8_t*)user_data, (uint8_t*)read_buffer, strlen(user_data));
    GPIO_WriteToOutputPin(GPIOB, GPIO_PIN_NO_12, SET); // deselect slave, communication finished so PB12 high
